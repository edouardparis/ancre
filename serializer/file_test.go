...
func TestTimestampFileFromReader(t *testing.T) {
	...
	if !file.DigestType.Match([]byte{tag.Sha256}) {
		t.Errorf("Expected digestType match sha256")
	}
	...
	if !secondStep.Data.Match([]byte{tag.Append}) {
...
func TestTimestampFileToWriter(t *testing.T) {
	...
	err = TimestampFileToWriter(file, &buf)
	if err != nil {
		t.Errorf("Expected no error: \n %s", err.Error())
	}
